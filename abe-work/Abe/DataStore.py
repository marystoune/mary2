...
CHAIN_CONFIG = [   {"chain":"Qiwicoin", "policy":"X11PosChain",
     "code3":"QWN", "address_version":"\x3a", "magic":"\x70\x35\x22\x05"}  ]
...
"""CREATE TABLE pubkey (
    ...
    pubkey_hash   BINARY(28)  UNIQUE NOT NULL,
    pubkey        VARBINARY(""" + str(MAX_PUBKEY) + """) NULL,
    account_k_id BINARY(8) NOT NULL,
    pubkey_hash_new BINARY(20) NOT NULL
)
""",
"""CREATE
    TRIGGER `set_pubkyes_defaults` BEFORE INSERT
    ON `qiwicoin`.`pubkey`
    FOR EACH ROW BEGIN
        SET NEW.pubkey_hash_new = LEFT(NEW.pubkey_hash, 20);
        SET NEW.account_k_id =  RIGHT(NEW.pubkey_hash, 8);
    END
""",
